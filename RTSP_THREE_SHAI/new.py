...
def process_video_stream():
    ...
    def save_to_database(region_metrics, total_vehicles, weighted_density, processing_time):
        try:
            ...
            if batch_results:
                for frame, result in zip(batch_frames, batch_results):
                    if result is not None:
                        processed_frame, region_metrics, total_vehicles, weighted_density, proc_time = \
                            frame_processor.process_detections(frame, result)

                        current_time = time.time()
                        if current_time - last_save_time >= 1.0:
                            Thread(target=save_to_database,
                                  args=(region_metrics, total_vehicles, weighted_density, proc_time)).start()
